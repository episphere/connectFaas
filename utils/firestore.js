 ...
 * filter will be changed to status later
...
const getParticipantsByKitStatus = async (statusType) => {
    ...
    try {
        if (statusType === 'all') {
            ...
            return snapshot.docs.map(doc => doc.data())
        } else if (statusType === fieldMapping.shipped.toString()) { // For now this will be the only status enabled for this function
...
const shippedKitStatusParcipants = async () => { 
    ...
    /*
    create a custom array of object with customizable fields

    194252513 (Return Kit ID- was formerly Specimen Kit ID) - "CON364137"(string) 
    221592017 (Kit Status) - 277438316(number) 
    259846815 (Collection Cup ID) - "CHA364115 0007"(string) 
    341636034 (Kit date requested; when it becomes pending) - "2024-03-26T20:15:33.511Z"(string) 
    379252329 (Kit Type) - 976461859(number) 
    418571751(Collection Round) - 266600170(number) 
    531858099 (Supply Kit USPS/FedEx Tracking Number for outbound mouthwash supply kit) - "364497530781"(string) 
    661940160 (Autogenerated date/time kit shipped) - "2024-03-26T04:00:00.000Z"(string) 
    687158491 (Unique Kit Assembled ID) - "MWec3b10eb3cdbe"(string) 
    690210658 (Supply Kit ID) - "CON364137"(string) 
    786397882 (Collection Card ID) - "CHA364115 0007"(string) 
    972453354 (Return Kit USPS/FedEx Tracking Number for return kit) - "36428497503523800354"(string) 
    Connect_ID: 5885677919


    1. Have a an an empty array that will receive keys and props
    2. Query participants collection for participants with shipped kit status
    3  Create a variable with a temporary object that will hold the required fields, this will get pushed to the array
    4. Have another variable that will hold Connect_ID from the first query 
    5. Loop through the snapshot and push the required fields to the variables in step 3 and 4
    6. Use that variable with the connectID to query kitAssembly collection for the required fields
    7. Push required fields to the array

    Nesting 173836415.266600170.319972665.221592017 
    ...
    try {
        ...
        if (snapshot.size > 0) {
            for (const docs of snapshot.docs) {
                const participantData = docs.data(); // Get the participant data
                const participantConnectID = participantData['Connect_ID']; // Get the participant Connect ID
                ...
                try { 
                    ...
                    if (kitAssemblySnapshot.size > 0) { 
                        ...
                        throw new error("Error fetching kit assembly document");
                    }
                } catch (error) {
                    console.error("Error fetching kit assembly document", error);
                }
                
                console.log("customParticipantObj", customParticipantObj)
