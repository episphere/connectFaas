...
const resetParticipantHelper = async (uid, saveToDb) => {
    ...
    const toDelete = {};
    let biospecimensToDelete = [];
    let obj = {};
    await db.runTransaction(async (transaction) => {
        console.log('Transaction start at %s', +new Date());
        ...
        console.log('Participant snapshot obtained at %s', +new Date());
        if (snapshot.size === 0) {
        ...
        console.log('prevUserData for %s', Connect_ID, JSON.stringify(obj, null, '\t'));
        obj = {
            state: {
                uid,
                ...defaultStateFlags
        ...
        console.log('New values for user %s', Connect_ID, JSON.stringify(obj, null, '\t'));

        // @TODO: Update DB with obj

        // @TODO: Get all secondary + tertiary data by directly running here vs. using helper methods
        // Then probably update to use transaction

        const userSurveyPromises = Object.keys(moduleConceptsToCollections).map((concept) => {
            return new Promise(async (resolve, reject) => {
                try {
                    console.log('Querying user surveys at %s', +new Date());
                    ...
                    if (docSnapshot.size > 0) {
                        toDelete[concept] = docSnapshot.docs.map(doc => doc.id);
                    }
                    console.log('Resolving user survey promises at %s', +new Date());
                    return resolve();
                } catch(err) {
                    reject(err);
                }
        ...
        const userNotificationsPromise = new Promise(async (resolve, reject) => {
            try {
                console.log('Querying user notifications at %s', +new Date());
                ...
                console.log('Resolving user notification promise at %s', +new Date());
                return resolve();
            } catch (err) {
                reject(err);
            }
        ...
        const userBiospecimenPromise = new Promise(async (resolve, reject) => {
            try {
                console.log('Querying user biospecimens at %s', +new Date());
                ...
                console.log('Resolving user biospecimens promise at %s', +new Date());
                return resolve();
            } catch (err) {
                reject(err);
            }
        ...
        const userCancerOccurrencesPromise = new Promise(async (resolve, reject) => {
            try {
                console.log('Querying user cancer occurrences at %s', +new Date());
                ...
                console.log('Resolving user cancer occurrences promise at %s', +new Date());
                return resolve();
            } catch (err) {
                reject(err);
            }
        ...
        const kitAssemblyPromise = new Promise(async (resolve, reject) => {
            try {
                console.log('Querying user kit assembly at %s', +new Date());
                ...
                console.log('Resolving user kit assembly at %s', +new Date());
                return resolve();
            } catch (err) {
                reject(err);
            }
        ...
        console.log('All read promises resolved at %s', +new Date());

        console.log('toDelete', toDelete);
        if (saveToDb) {
            const participantDoc = db.collection('participants').doc(userId);
            // @TODO: This batching isn't right. See: https://firebase.google.com/docs/firestore/manage-data/delete-data#node.js_2
            console.log('Running delete lookup promises at %s', +new Date());
            const deleteDocs = await Promise.all(Object.keys(toDelete).map((docType) => {
            ...
            console.log('Done with delete lookup promises at %s, updating participant', +new Date());
            ...
            const deletePromises = deleteDocs.map(doc => doc && doc.ref ? transaction.delete(doc.ref) : Promise.resolve());
            await Promise.all([participantUpdatePromise, Promise.all(deletePromises)]);
            console.log('Done with deletePromises and participantUpdatePromise at %s', +new Date());
        }
        console.log('Final transaction returning at %s', +new Date());
        return;
    });
...
// Reference: https://firebase.google.com/docs/firestore/manage-data/delete-data#node.js_2
