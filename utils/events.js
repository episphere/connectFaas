const importToBigQuery = async (event, context) => {
    ...
    const url = `gs://connect_firestore_backup_stage/${gcsEvent.name}`;
    console.log(url)
    ...
    const bucketName = 'connect_firestore_backup_stage';
...
const firestoreExport = async (event, context) => {
    ...
    const bucket = 'gs://connect_firestore_backup_stage';
